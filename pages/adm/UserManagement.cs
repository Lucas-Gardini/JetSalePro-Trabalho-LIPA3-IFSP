...
namespace JetSalePro {
    public partial class UserManagement : KryptonForm {
		...
		private async void AtLoad(object sender, User toSearch = null)
		{
			...
			if (toSearch != null)
			{
				...
				if (toSearch.Situacao != false)
				{
					if (where.Length > 7) where += " AND";
					where += $" situacao = {toSearch.Situacao}";
				}

				if (toSearch.Adm != false)
				{
					if (where.Length > 7) where += " AND";
					where += $" adm = {toSearch.Adm}";
				}
			...
			// Obtendo os usuários
			var data = await User.GetUsers(where);
